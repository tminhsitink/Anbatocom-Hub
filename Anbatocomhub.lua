...
local Window = Fluent:CreateWindow({
    Title = "AnBaToCom Hub",
    SubTitle = "by Long Nhat and Tan Minh",
    TabWidth = 160,
...
Tabs.Race:AddParagraph({
    Title = "Misc Race",
    Content = "Auto Acient One(Farm And Buy)"
...
if _G.NoDashCooldown then
    for a, a in next, getgc() do
        if game.Players.LocalPlayer.Character.Dodge then
            if typeof(a) == "function" and getfenv(a).script == game.Players.LocalPlayer.Character.Dodge then
                for b, c in next, getupvalues(a) do
                    if tostring(c) == "0.4" then
                        repeat
                            wait(.1)
                            setupvalue(a, b, 0)
...
Local ToggleFindMoon = Tabs.Hop:AddToggle("Toggle FindMoon"), {Title = "Find Full Moon"}  Default = false })
